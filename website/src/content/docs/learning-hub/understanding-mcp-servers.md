...
- **Microsoft Entra ID (Azure AD)**: MCP servers that authenticate via Microsoft Entra ID are fully supported. Once you complete the initial login, the CLI caches the authentication and **will not show the consent screen on subsequent connections** — you authenticate once per session rather than every time the server reconnects. On **Windows (v1.0.81+)**, Entra ID authentication uses the OS authentication broker (WAM), so sign-in typically happens automatically with no prompt at all when you are already signed into Windows with the same account. Other platforms continue to use the browser flow.
