...
- **Microsoft Entra ID (Azure AD)**: MCP servers that authenticate via Microsoft Entra ID are fully supported. Once you complete the initial login, the CLI caches the authentication and **will not show the consent screen on subsequent connections** — you authenticate once per session rather than every time the server reconnects. On **Windows**, remote MCP servers protected by Microsoft Entra ID now sign in through the OS authentication broker (WAM), which typically completes silently with no prompt at all if you are already signed in to Windows with your work account. Other platforms and machines without the broker fall back to the existing browser flow automatically.
