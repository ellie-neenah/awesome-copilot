...
## Canvas extensions in GitHub Copilot CLI

*(v1.0.71+)* Canvas extension support is also available in **GitHub Copilot CLI**, not only in the Copilot app. When a canvas extension is installed (from a marketplace plugin or a local plugin directory), the CLI can host the extension's interactive panel alongside a chat session.

This means canvas extensions you create for the Copilot app can also be surfaced in terminal-based CLI workflows — useful for teams that run automations or coding-agent sessions from the CLI and still want the structured, interactive surface that a canvas provides.

Extensions installed via plugins that include an `extensions/` entry are automatically available in both the CLI and the Copilot app. No separate configuration is required.
