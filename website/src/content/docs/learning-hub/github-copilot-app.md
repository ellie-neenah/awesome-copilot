...
If you are working in a GitHub Copilot CLI session, use the `/app` command to open that session directly in the GitHub Copilot desktop app (requires app version 1.1.3 or later):

```
/app
```

This opens the app focused on the current session and its repository, rather than landing on the Home screen with the wrong folder.
