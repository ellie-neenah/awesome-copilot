...
- **First-party plugins update automatically** *(v1.0.78+)*: plugins installed from the official `copilot-plugins` marketplace are automatically updated to the latest version at session start. For other plugins, run `copilot plugin update` to get the latest improvements manually.
