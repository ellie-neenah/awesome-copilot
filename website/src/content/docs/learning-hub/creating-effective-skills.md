...
**Skill and agent discovery from extra directories** (v1.0.81+): When you start the CLI with the `--add-dir` flag, skills and custom agents are automatically discovered from those additional directories — no manual install step required:

```bash
copilot --add-dir ~/shared-skills/ --add-dir ~/team-agents/
```

This is useful for sharing a collection of skills across multiple projects without copying them into each repository's `.github/skills/` folder.
