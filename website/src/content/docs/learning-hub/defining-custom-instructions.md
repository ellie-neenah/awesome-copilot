...
In the Copilot CLI, you can run `/instructions` to see each loaded instruction file listed individually (v1.0.81-6+). This makes it easy to confirm which instruction files are active in the current session and identify any that may not have been picked up.
